...
def create_gui():
    ...
    folder_button = tk.Button(
        root, text="选择文件夹", command=lambda: select_folder(folder_entry)
    )
    ...
    markdown_button = tk.Button(
        root, text="选择文件夹", command=lambda: select_folder(markdown_entry)
    )
    ...
    word_button = tk.Button(
        root, text="选择文件夹", command=lambda: select_folder(word_entry)
    )
    ...
    proxy_checkbox = tk.Checkbutton(
        root,
        text="使用代理",
        variable=proxy_var,
        command=lambda: toggle_proxy_entry(
            http_proxy_entry, https_proxy_entry, all_proxy_entry, proxy_var
        ),
    )
    ...
    markdown_radio = tk.Radiobutton(
        root, text="生成 Markdown 文件", variable=output_format_var, value="markdown"
    )
    word_radio = tk.Radiobutton(
        root, text="生成 Word 文件", variable=output_format_var, value="word"
    )
        ...
        if filename.endswith(".pdf"):
            ...
            try:
                ...
                if output_format == "markdown":
                    save_summaries_to_markdown(
                        filename, summary, markdown_output_folder
                    )
