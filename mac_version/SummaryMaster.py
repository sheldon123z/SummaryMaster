...
def create_gui():
    ...
    proxy_checkbox = tk.Checkbutton(root, text="使用代理", variable=proxy_var)
    ...
    markdown_radio.grid(row=5, column=1, padx=10, pady=10)
    word_radio.grid(row=5, column=2, padx=10, pady=10)
...
def start_processing(folder_path, markdown_output_folder, word_output_folder, api_key, base_url, use_proxy, output_format, progress_bar, root):
    # 设置代理
    if use_proxy:
